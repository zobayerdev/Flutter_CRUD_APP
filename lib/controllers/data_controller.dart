...
class DataController implements DataRepository {
  ...
  List<ProductModel> sortProductHighToLow(List<ProductModel> productList) {
    ...
    sortedList.sort((firstModel, secondModel) =>
        (int.tryParse(secondModel.totalPrice) ?? 0)
            .compareTo((int.tryParse(firstModel.totalPrice) ?? 0)));
    return sortedList;
  }
  ...
  List<ProductModel> sortProductLowToHigh(List<ProductModel> productList) {
    List<ProductModel> sortedList = List.from(productList);
    sortedList.sort((firstModel, secondModel) =>
        (int.tryParse(firstModel.totalPrice) ?? 0)
            .compareTo((int.tryParse(secondModel.totalPrice) ?? 0)));
    return sortedList;
  }
}
